...
import {
  createTableSchema,
  tableAreaSchema,
  tableImportFileSchema,
  tableImportRowsSchema,
  tableManageQuerySchema,
  updateTableSchema,
...
export class TablesController {
  static async manage(req: Request, res: Response, next: NextFunction) {
    try {
      res.json({ data: await TablesService.manage(tableManageQuerySchema.parse(req.query)) });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      res.status(201).json({ data: await TablesService.create(createTableSchema.parse(req.body), req.user?.id, req.user?.name) });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      res.json({ data: await TablesService.update(parseId(req.params.id), updateTableSchema.parse(req.body), req.user?.id, req.user?.name) });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async areas(_req: Request, res: Response, next: NextFunction) {
    try {
      res.json({ data: await TablesService.areas() });
    } catch (error) {
      next(error);
    }
  }

  static async saveArea(req: Request, res: Response, next: NextFunction) {
    try {
      const id = req.params.id ? parseId(req.params.id) : null;
      res.status(id === null ? 201 : 200).json({ data: await TablesService.saveArea(id, tableAreaSchema.parse(req.body), req.user?.id, req.user?.name) });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async export(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const buffer = await TableTransferService.export(tableManageQuerySchema.parse(req.query), format);
      res.type(format === 'csv' ? 'text/csv; charset=utf-8' : 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet').attachment(`Phong_ban.${format}`).send(buffer);
    } catch (error) {
      next(error);
    }
  }
  ...
  static async importTemplate(_req: Request, res: Response, next: NextFunction) {
    try {
      res.type('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet').attachment('Mau_phong_ban.xlsx').send(TableTransferService.template());
    } catch (error) {
      next(error);
    }
  }
  ...
  static async previewImport(req: Request, res: Response, next: NextFunction) {
    try {
      const input = tableImportFileSchema.parse(req.body);
      res.json({ data: await TableTransferService.preview(input.fileName, input.fileBase64) });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async commitImport(req: Request, res: Response, next: NextFunction) {
    try {
      res.status(201).json({ data: await TableTransferService.commit(tableImportRowsSchema.parse(req.body).rows, req.user?.id, req.user?.name) });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async getTableByTableNumber(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
