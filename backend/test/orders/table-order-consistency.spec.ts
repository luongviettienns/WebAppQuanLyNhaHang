...
describe('multiple unpaid orders on one table', () => {
  ...
  it('does not rewrite payment timestamps when a paid order is paid again', async () => {
    ...
    const result = await OrdersService.payOrder(1, { paymentMethod: 'CASH' });

    expect(orderUpdate).not.toHaveBeenCalled();
    expect(result.order.paidAt).toEqual(paidOrder.paidAt);
  });
});
